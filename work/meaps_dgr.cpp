...
// [[Rcpp::export]]
NumericVector meaps_dgr(IntegerVector j_dist,
  ...
  for (int from = 0; from < N; ++from) {
    ...
    for (int it = p_modds(from); it < p_modds(from + 1L); ++it ) {
      les_odds.insert(std::make_pair(j_modds(it), x_modds(it)));
    }
    
    for (int it = p_dist(from); it < p_dist(from + 1L) ; ++it) {
      int rang_to = les_rangs[ it - p_dist(from) ] - 1L;
      arrangement[from][rang_to] = j_dist(it);
      
      if (les_odds.find( j_dist(it) ) == les_odds.end()) {
        odds[from][rang_to] = 1; //La valeur par défaut pour modds est un odds ratio fixé à 1.
      } else {
        odds[from][rang_to] = exp(les_odds[j_dist(it)]); //Attention : on veut des log en entrée.
      }  
    }
  ...
  #pragma omp parallel for num_threads(ntr)                                                          \
    ...
    for (int iboot = 0; iboot < Nboot; ++iboot) {
      ...
      for (auto from: theshuf) {
        ...
        repartition = repartir_actifs(placeslibres, attractivites, odds[from], fcpp[from], actifspartant, seuil_newton);
    ...
    int position;
    NumericVector resultat(j_dist.size());
    for (int from = 0; from < N; ++from) {
      for(int k = 0; k < p_dist(from + 1L) - p_dist(from); ++k) {
        position = *std::find( j_dist.begin() + p_dist(from), j_dist.begin() + p_dist(from + 1L), arrangement[from][k]);

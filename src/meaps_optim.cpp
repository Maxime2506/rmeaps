...
//' Fonction de pénalité "marche" : vaut 1 sur un rayon fixé, et decru au-delà.
 ...
 NumericMatrix meaps_optim_cpp(IntegerVector jr_dist, 
   ...
   std::vector<int> jr_lor = as< std::vector<int> >(jr_odds);
...
#pragma omp parallel for num_threads(ntr) \
     shared(Nboot, N, ishuf, emploisinitial, fcpp, actifscpp, _xr_dist, _jr_dist, _p_dist, xr_lor, jr_lor, _row_group, _col_group) reduction(vsum : liaisons)
#endif

...
//' Fonction de pénalité "marche" : vaut 1 sur un rayon fixé, et fattract au-delà.
 inline double marche(double x, const double rayon, const double fattract) {
   if (x < rayon) return 1;
   return fattract;
 }
 ...
 //' @param attraction Choix de la fonction d'attraction appliquée à l'accessibilité.
     ...
     for (int iboot = 0; iboot < Nboot; ++iboot) {
       ...
       for (auto from : theshuf) {
         ...
         for (std::size_t k = 0; k < k_valid; ++k) {
           ...
           liaisons[ curseur_ligne + col_group[jr_dist[debut + k]] ] += repartition[k];
         }
       }
     }

...
//' c'est-à-dire le rang moyen sur les shufs juste avant saturation.
 ...
 // [[Rcpp::export]]
 List meaps_tension_alt(IntegerMatrix rkdist,
   ...
   // on ajoute 2 lignes pour la tension
   std::vector<double> liaisons((N+2L)*K, 0.0);
...
#pragma omp parallel for num_threads(ntr)                                                          \
     ...
     for (int iboot = 0; iboot < Nboot; ++iboot) {
       ...
       for (auto i: theshuf) {
         ...
         // la tension sera calculée hors fuite (contrairement à la version meaps_tension)
         rang_actifs += actifspartant;
         ...
         // plus un bout de code pour la tension
         for(int k = 0; k < k_valid ; ++k) {
           if ((placeslibres[k]-repartition[k]) < seuil_dispo && placeslibres[k] > seuil_dispo) { 
             liaisons[ N*K + arr[k] ] += 1.0; 
             liaisons[ (N + 1L) * K + arr[k] ] += rang_actifs;
           }
